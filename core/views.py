...
from .models import NewsAndEvents, ActivityLog, Session, Semester, LearningActivity
...
def home_view(request):
    ...
    log_learning_activity(
        request.user,
        LearningActivity.ACTIVITY_LOGIN,
        description="Visited home/news page",
    )
    context = {
        "title": "News & Events",
        "items": items,
...
def dashboard_view(request):
    ...
    activity_labels = [d.strftime("%m-%d") for d in last_7_days]
    activity_counts = [
        LearningActivity.objects.filter(created_at__date=d).count()
